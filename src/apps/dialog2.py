...
class AIConversation:
    def __init__(self,id,max_loop) -> None:
        ...
        self.diagnostic_prompt = PromptTemplate(
            ...
            Also give a rating of how likely you think each diagnosis is. The value should be between 1 to 10, with 10 being the most likely.
            For each diagnosis, provide a brief explanation of why you think it is likely.
            The diagnoses should be based on ICD-10 codes. privide the link to the ICD-10 code for each diagnosis.
        ...
        self.question_to_clarify_issue_prompt =PromptTemplate(
            ...
            If the issue may caused by communicable diseases, include a question that asks the human who and when they were in contact with the person. 
            Optional: Consider asking the human about medicines they are taking.
            ...
            Impoartant: The questions should help narrow down the possible causes of the health issue.
            ...
            ** Example 1: no health issues mentioned **
            {{
            "question_to_clarify": {{
                "1": {{"question":"What health issues are you experiencing?", "selective_answers": ["Fever", "Cough"]}}
                }}
            }}
            **You will reply only with the JSON itself, and no other descriptive or explanatory text.**
            """
        ...
        self.decision_prompt = """
            ...
            - **Review** the entire conversation history, consider on all messages, including their answers to your questions.
    ...
    def question_to_clarify_issue(self,state):
        ...
        question_input=self.question_to_clarify_issue_prompt.invoke({"health_issues":self.health_issues,"medical_history":self.medical_history})
        messages = state["messages"]
    ...
    def should_continue(self,state):
        messages = state["messages"]
        messages[0].content = self.decision_prompt
        # If there is no function call, then we finish
        response=self.model.invoke(messages)
        # print("decision response:",response)
        if "yes" in response or self.loop_num>=self.max_loop:
...
def receive_human_answer():
    ...
    for event in conv.app.stream(None, conv.config, stream_mode="values"):
        event["messages"][-1].pretty_print()
    feeebackFromAI=event["messages"][-1].content
